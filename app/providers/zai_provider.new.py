                                ...
                                # Memproses konten jawaban
                                    ...
                                    if data.get("usage"):
                                        ...
                                        if has_tools:
                                            ...
                                            # Jika tidak ada tools, tetap coba ekstrak format dari interface TypeScript
                                            tool_calls, cleaned_content = parse_and_extract_tool_calls(buffered_content)
                                            if tool_calls is None and cleaned_content != buffered_content:
                                                # Hanya pesan biasa yang telah dibersihkan, tidak perlu dikirim sebagai tool_calls
                                                buffered_content = cleaned_content
                                            elif tool_calls is not None:
                                                # Ada tool_calls yang diekstrak dari interface, gunakan itu
                                                buffered_content = cleaned_content  # Gunakan konten yang telah dibersihkan dari tool JSON
                                        ...
                                        if tool_calls:
                                            ...
                                            # Tapi cek apakah buffered_content mengandung format interface yang telah diproses
                                            _, cleaned_content = parse_and_extract_tool_calls(buffered_content)
    ...
    async def _handle_non_stream_response(
        ...
        if all_content_buffer.strip():
            ...
            if tool_calls:
                ...
                # Jika tidak ada tool_calls tetapi konten telah dibersihkan (misalnya dari format pesan interface), gunakan cleaned_content
                if cleaned_content != all_content_buffer:
                    self.logger.info(f"📝 Cleaned message content from interface format")
                    final_content = cleaned_content
